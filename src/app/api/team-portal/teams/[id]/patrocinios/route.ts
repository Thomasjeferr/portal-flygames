...
/** Lista patrocínios do time (empresas + torcedores) para a área do time. */
export async function GET(
  ...
  const [sponsorEarnings, planEarnings] = await Promise.all([
    prisma.teamSponsorshipEarning.findMany({
      ...
      include: {
        sponsorOrder: {
          include: { sponsorPlan: { select: { name: true } } },
        },
      },
  ...
  const sponsorItems: Row[] = sponsorEarnings.map((e) => ({
    id: e.id,
    tipo: 'empresa',
  ...
  const planItems: Row[] = planEarnings.map((e) => {
