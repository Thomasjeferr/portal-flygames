...
/** Upload público de escudo no cadastro de time (sem auth). */
export async function POST(request: NextRequest) {
  try {
    ...
    const safeName = `team-crest-${Date.now()}-${Math.random().toString(36).slice(2)}${claimedExt}`;

    // Em produção (Vercel): disco é read-only → usar Vercel Blob
    if (process.env.BLOB_READ_WRITE_TOKEN) {
      const blob = await put(`team-crests/${safeName}`, file, { access: 'public', addRandomSuffix: false });
