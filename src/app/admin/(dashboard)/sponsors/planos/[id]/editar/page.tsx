...
export default function EditSponsorPlanPage() {
  ...
  useEffect(() => {
    fetch(`/api/admin/sponsor-plans/${id}`)
      .then((r) => r.json())
      .then((data) => {
        if (data.error) {
          setError(data.error);
        ...
        const flags = data.featuresFlags && typeof data.featuresFlags === 'object' ? data.featuresFlags : {};
        const mergedFlags = Object.fromEntries(
          DEFAULT_FEATURE_FLAGS.map((f) => [f.key, flags[f.key] ?? true])
        );
        setForm({
          name: data.name ?? '',
  ...
  if (error && !form.name) {
    return (
      <div className="max-w-2xl mx-auto px-6 py-10">
        ...
        <Link href="/admin/sponsors/planos" className="text-netflix-light hover:text-white mt-4 inline-block">
          Voltar aos planos
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-10">
      ...
      <h1 className="text-2xl font-bold text-white mb-6">Editar plano de patrocínio</h1>
            ...
            {form.benefits.map((b, i) => (
              <div key={i} className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-4 py-2 rounded bg-netflix-gray border border-white/20 text-white focus:outline-none focus:ring-2 focus:ring-netflix-red"
                />
                <button
                  type="button"
