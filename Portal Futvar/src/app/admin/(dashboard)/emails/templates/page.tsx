...
export default function AdminEmailTemplatesPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-10">
          ...
          <div key={t.id} className="bg-netflix-dark border border-white/10 rounded-lg p-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-netflix-light/70 mt-1">
                Variaveis: {TEMPLATE_VARS[t.key]?.map((v) => `{{${v}}}`).join(', ') ?? '-'}
              </p>
      ...
      {templates.length === 0 && (
        <p className="text-netflix-light">Nenhum template. Execute: <code className="bg-netflix-gray px-2 py-1 rounded">npx tsx prisma/seed-emails.ts</code></p>
      )}
      <Link href="/admin/emails/settings" className="inline-block mt-6 text-netflix-light hover:text-white text-sm">Configuracoes</Link>
    </div>
  );
}
