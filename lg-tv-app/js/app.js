/**
 * Fly Games - LG webOS TV app
 * Mesmo fluxo: QR no celular, catálogo, player. API_BASE = flygames.app quando rodando no .ipk.
 */
(function () {
  ...
  function showScreen(id) {
    ...
    if (id === 'screen-player') {
      document.body.classList.add('screen-player-active');
    } else {
      document.body.classList.remove('screen-player-active');
    }
  }
  ...
  function loadFooterSponsors() {
    var container = document.getElementById('tv-footer-sponsors');
    if (!container) return;
    fetch(API_BASE + '/api/public/sponsors', { credentials: 'omit' })
      ...
      .then(function (list) {
        ...
        list.slice(0, 12).forEach(function (s) {
          ...
          if (s.logoUrl) {
            ...
            a.href = s.websiteUrl || '#';
            if (s.websiteUrl) a.target = '_blank';
            a.rel = 'noopener';
            ...
            wrap.className = 'tv-footer-sponsor-name';
          }
